...
@Component({
    ...
    <div class="ng-bottom-sheet-panel">
      ...
      <div class="ng-bottom-sheet-panel__actions">
        @for (action of data.actions; track action.value) {
          <button
            type="button"
            class="ng-bottom-sheet-panel__action"
            (click)="dismiss(action.value)"
          >
  ...
  styles: [
    `
      .ng-bottom-sheet-panel {
        display: grid;
        gap: 16px;
        padding: 8px 4px 12px;
      }
      .ng-bottom-sheet-panel__eyebrow {
        margin: 0;
        color: #4f46e5;
        font-size: 0.75rem;
      ...
      .ng-bottom-sheet-panel h3 {
        margin: 0;
        font-size: 1.35rem;
      }
      .ng-bottom-sheet-panel__description {
        margin: 0;
        color: #475569;
      }
      .ng-bottom-sheet-panel__highlight {
        padding: 12px 14px;
        border-radius: 16px;
      ...
      .ng-bottom-sheet-panel__actions {
        display: grid;
        gap: 10px;
      }
      ...
      .ng-bottom-sheet-panel__action:hover {
        background: #eef2ff;
      }
      .ng-bottom-sheet-panel__action-copy {
        display: grid;
        gap: 4px;
      }
      .ng-bottom-sheet-panel__action-copy span {
        font-weight: 600;
        color: #0f172a;
      }
      .ng-bottom-sheet-panel__action-copy small {
        color: #64748b;
      }
    `,
  ],
...
class NgBottomSheetPanelComponent {
  ...
  private readonly ref = inject(
    MatBottomSheetRef<NgBottomSheetPanelComponent, string | undefined>,
  );
...
export class NgBottomSheetComponent {
  ...
  readonly description = input(
    'Open a polished mobile action sheet for tier upgrades, delivery controls, or account tasks.',
  );
  readonly highlight = input(
    'Smart defaults, richer copy, and concierge-style actions included.',
  );
  ...
  readonly actions = input<NgBottomSheetAction[]>([
    {
      ...
      description: 'Unlock faster support and priority processing.',
      icon: 'north_east',
      tone: 'primary',
    },
    {
      value: 'pause',
      label: 'Pause renewals',
      description: 'Keep access through the current billing cycle.',
      icon: 'pause_circle',
      tone: 'warning',
    },
    {
      ...
      description: 'Escalate this request to a human specialist.',
      icon: 'support_agent',
      tone: 'success',
    },
  ]);
