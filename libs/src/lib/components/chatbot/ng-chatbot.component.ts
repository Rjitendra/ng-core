...
@Component({
  ...
  template: `
    <section
      class="ng-chatbot"
      [class.ng-chatbot--compact]="density() === 'compact'"
    >
      <header class="ng-chatbot__header">
        <div class="ng-chatbot__brand">
          ...
          <div class="ng-chatbot__heading">
            ...
            <h3>{{ title() }}</h3>
            <span
              class="ng-chatbot__status"
              [class]="'ng-chatbot__status--' + status()"
            >
      ...
      <div class="ng-chatbot__conversation">
        @for (message of messages(); track message.id) {
          <article
            class="ng-chatbot__message"
            [class]="'ng-chatbot__message--' + message.role"
          >
            ...
            <div class="ng-chatbot__bubble">
              <div class="ng-chatbot__meta">
                <strong>{{
                  message.author || defaultAuthor(message.role)
                }}</strong>
      ...
      <div class="ng-chatbot__composer">
        <label class="ng-chatbot__composer-label" [attr.for]="inputId()">{{
          composerLabel()
        }}</label>
        <textarea
  ...
  styles: [
    `
      ...
      .ng-chatbot__status--online {
        background: #dcfce7;
        color: #166534;
      }
      .ng-chatbot__status--thinking {
        background: #fef3c7;
        color: #92400e;
      }
      .ng-chatbot__status--offline {
        background: #fee2e2;
        color: #991b1b;
      }
...
export class NgChatbotComponent {
  ...
  readonly supportingText = input(
    'Use this control for guided support, concierge workflows, or embedded product copilots.',
  );
  ...
  readonly messages = input<NgChatbotMessage[]>([
    {
      ...
      author: 'Concierge AI',
      content:
        'Welcome back. I can help summarize activity, draft replies, or route premium support requests.',
      timestamp: 'Just now',
    },
  ]);
  readonly quickActions = input<NgChatbotQuickAction[]>([
    {
      ...
      prompt: 'Summarize this account for leadership.',
      icon: 'summarize',
    },
    {
      id: 'draft',
      label: 'Draft client reply',
      prompt: 'Draft a polished update for the client.',
      icon: 'edit_square',
    },
    {
      id: 'route',
      label: 'Escalate support',
      prompt: 'Escalate this conversation to premium support.',
      icon: 'support_agent',
    },
  ]);
  ...
  readonly helperText = input(
    'Shift the copy and quick actions to fit your support or copilot workflow.',
  );
  readonly rows = input(4);
  readonly disabled = input(false);
  readonly density = input<'default' | 'compact'>('default');
  readonly inputId = input(
    `ng-chatbot-${Math.random().toString(36).slice(2, 9)}`,
  );
